...
proc opADC(cpu): uint8 =
  with cpu:
    ...
    flags.V = (not ((a.uint16 xor fetched.uint16) and (
            a.uint16 xor tmp)) and 0x0080'u16).bool
...
proc opPHP(cpu): uint8 =
  with cpu:
    cpu.write(0x0100'u16 + sp, cast[uint8](flags) or cast[uint8](
            CPUFlag.B) or cast[uint8](CPUFlag.U))
    flags.B = false
    flags.U = false
    dec sp
  result = 0
...
proc opROL(cpu): uint8 =
  with cpu:
    discard cpu.fetch()
    let tmp: uint16 = (fetched.uint16 shl 1) and (
            if flags.C: 1'u16 else: 0'u16)
    flags.C = (tmp and 0xFF00'u16).bool
    flags.Z = (tmp and 0x00FF'u16) == 0x0000'u16
    flags.N = (tmp and 0x0080'u16).bool
    if lookup[opcode].mode == amIMP:
...
proc opROR(cpu): uint8 =
  with cpu:
    discard cpu.fetch()
    let tmp: uint16 = (if flags.C: 1'u16 shl 7 else: 0'u16) or (
            fetched.uint16 shr 1)
